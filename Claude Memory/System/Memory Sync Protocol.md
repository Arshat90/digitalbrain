...
## Слепая зона: облачные чаты (v3.9, 2026-09-04)

`mcp__session_info__list_sessions` отдаёт **только локальные сессии** этой машины (`local_*`, хранятся в `AppData\Roaming\Claude\local-agent-mode-sessions`). Облачные чаты claude.ai в этот список не попадают — их транскрипты остаются на сервере. Следовательно, сессионный обход их не видит в принципе, и никакое расширение `limit` этого не лечит.

**Прецедент.** Переговоры [[ФК Кайрат — 1xBET]] шли 27.07–19.08.2026 в облачных чатах и дали 26 документов в `Downloads` — в vault за три недели не попало ни одной записи. Обнаружено 2026-09-04 при попытке найти эту работу в памяти. Проверка: grep по всем 721 локальному транскрипту (с 23.04.2026) — ни одного упоминания переговоров, при том что локальных сессий в тот период было 4–6 в день.

**Компенсация — обход по файлам.** Сессия может быть невидима, файл — нет. `daily-memory-sync` §3d (инвариант 20) каждый прогон сканирует `Downloads`, `Documents\Claude\Projects` и `Desktop` по собственному watermark в [[Sweep State]]. Подробная спецификация и скрипты — `C:\Users\LENOVO\Documents\Claude\Scheduled\deliverables-sweep\`.

**Второй контур — сама сессия.** Облачная сессия, пока компьютер на связи, имеет доступ к obsidian-rest и пишет в vault сама — это закреплено в личном скиле `arshat-work-style` §11. Push-блок такой сессии пишется с пометкой `session: cloud Cowork, вне list_sessions` — реконсилиация по префиксам для них не работает, дедуп — по содержанию changelog-записи.
